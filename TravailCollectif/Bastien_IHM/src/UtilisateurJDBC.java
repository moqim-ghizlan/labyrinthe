...
public class UtilisateurJDBC {
    ...
    public boolean testerConnexion(String identifiant, String mdp) throws SQLException {
        ...
        if(rs.next()){
            this.uti = new Utilisateur(rs.getInt("idut"), rs.getString("pseudout"),rs.getString("emailut"),rs.getString("mdput"),rs.getString("activeut").charAt(0),rs.getString("idro"),rs.getBlob("avatarut"));
            return true;
        }
        return res;
    }
    public void creationCompte(String pseudo, String mdp,String adresse, Blob img) throws SQLException{
        ...
        PreparedStatement pst = this.connexionMySQL.prepareStatement("insert into UTILISATEUR values(?,?,?,?,?,?,?);");
        rs.next();
        int newId = rs.getInt("maxId");
        pst.setInt(1, newId+1);
        pst.setString(2, pseudo);
        pst.setString(3, adresse);
        pst.setString(4, "password('"+mdp+"')");
        pst.setString(5,"O");
        pst.setBlob(6, img);
        pst.setInt(7,2);
